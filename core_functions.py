...
class log(logging.Logger):
    ...
    def __init__(self, log_name, path, cwd, aux = None, console = True):
        """
        ...
        self.running_file = path.split('/')[-1]
        self.cwd = cwd
        self.log_file = f"{self.file_name}.log" 
    ...
    def print_basic_info(self):
        ...
        self.info("Running %s" % self.running_file)
...
def parameter_finder(target_list, search_list, msgflag=False, exact=False):
    ...
    if msgflag == True:
        length = len(indexes)
        if length > 1:
            print("There were several ocurrences")
        if length == 0:
...
def read_data_file(input_file):
    ...
    with open(input_file, 'r') as data_file:
        ...
        # Goes back to the last line of the header
        data_file.seek(last_pound_pos+1)

        if header_lines == 0:
            data = pd.read_csv(data_file, sep=" ", header=None).dropna(
                axis=1, how='all')
            ...
            if len(data_2) == 0:
                ...
                # If there is a line containing the number of particles,
        ...
        if np.shape(output)[1] == 1:
            ...
            if sort == True:
                ...
                columns = np.column_stack(
                    ([unsorted, np.arange(0, len(unsorted))]))
...
def set_plot_appearance(presentation_type=False):
    ...
    if presentation_type == False:
        ...
        plt.rcParams['axes.prop_cycle'] = cycler(
            linestyle=['-', '--', '-.']) * cycler(color=list('rbkgcmy'))
...
def get_size_dfs(dfs, names, key = None, print_size = True):
    """
    Return the size of an array of dataframes, if a key is provided, returns the size for each unique entry for that key as well
    Args:
        dfs = list of DataFrames
        names = list of names of the dfs
        key = to count the number of elements for each entry in the key column
    """
    for i, df in enumerate(dfs):
        ...
        if key:
            df_size_key = df.groupby(key).size()
            
            if print_size == True:
                print("The number of rows in the %s df is %s" %
                      (names[i], n))
                print(df_size_key)
            return n, df_size_key
...
def change_legend_alpha(ax, alpha = 1):
    """
    Sets the legend for all plots to the given value
    """
